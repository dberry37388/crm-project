...
const saveForm = () => {
    ...
    form.post(route('api.v1.contact.attach-company', {contact: props.contactId, company: form.company}), {
            errorBag: 'attachCompany',
            preserveScroll: true,
...
<template>
    <SlideoverWithTitle :show="show">
        ...
        <template #content>
            <div class="pt-6 pb-5">
                <div>
                    Use the tool below to associate this contact with an existing or new company.
                </div>
            </div>

            <div class="mt-5">
                <TeamCompaniesComboBox label="Search for a company to associate this contact with." v-model="form.company" />
            </div>
        </template>
